function [Fsol,Fval,trials] = ProcessDesign_AM(Matrix,x_cut,layerID,ProcessParam,...
...
options = optimoptions('surrogateopt','PlotFcn','surrogateoptplot',...
    'InitialPoints',x_initial,'UseParallel',true,'MaxFunctionEvaluations',100);
...
%                 end
        ...
        for num = sum(vector(1:layer-1,1))+1:sum(vector(1:layer-1,1))+vector(layer,1)
            ...
            if mod(layer,2)==0 % for even layers
                ...
                % check if array element is empty or not
                chk = size(save_T,2);
