function[Temp_2] = FDM_TempModel_GSA_fil2(ProcessParam,MaterialParam)

%Process Variables
x=ProcessParam(1);
T_L = ProcessParam(2); %Extrusion temperature (ºC)
T_E = ProcessParam(3); %Temperature of the envelope (ºC)
v = ProcessParam(4); % printer speed/Velocity of the extrusion head (m/sec)
%Filament dimensions
Wid = ProcessParam(5); % Layer Thickness (meters)
Ht = ProcessParam(6); % Layer Height (meters)
L = ProcessParam(7); % Length of the filament (meters)
