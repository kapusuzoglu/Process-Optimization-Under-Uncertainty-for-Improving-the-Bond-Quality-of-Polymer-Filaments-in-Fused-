function index = SingleLoop_FirstOrder_Alg2( input, output, ndomain, varargin)
...
%% compute indices
VY = var(output);
VarY_local = zeros(ndomain, nd, size(output,2));
for i = 1:nd
...
VarY_local = VarY_local(:,:,2:end);
VY = VY(2:end);

for jj = 1:size(output,2)-1
    index(jj,:) = 1-mean(VarY_local(:,:,jj))/VY(1,jj);
end
end